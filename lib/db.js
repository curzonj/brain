...
const { sublevel, nested, base, rdfStore } = require('../lib/leveldb');
const N3 = require('n3');
...
module.exports = {
  ...
  async pokeBear(topicId) {
    const shortId = topicId.slice(1);

    const tuples = await rdfStore.get({ subject: namedNode("https://curzonj.github.io/brain/#"+shortId) });
    console.log(tuples);
  },
  ...
  async sync(config) {
    ...
    if (!lastSeq) {
      ...
      await importTopicsToLevelDB(remoteDb, lastSeq);
    } else {
...
function unstringifyQuad(json) {
  return quad(
    RdfString.stringToTerm(json.subject),
    RdfString.stringToTerm(json.predicate),
    RdfString.stringToTerm(json.object),
    RdfString.stringToTerm(json.graph),
  );
}

async function importTuplesToQuadstore(remoteDb) {
  ...
  const quads = rows.map((row) => unstringifyQuad(row.doc) );

  await rdfStore.put(quads);
}

async function importTopicsToLevelDB(remoteDb, lastSeq) {
  const { rows, update_seq: resultSequence } = await remoteDb.allDocs({
    include_docs: true,
    startkey: `$/topics/`,
    endkey: `$/topics/\uFFF0`,
