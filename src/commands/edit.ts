...
import { applyEditorChanges, buildEditorStructure, editFile, sortedYamlDump  } from "../cli/editor";
...
class EditCommand extends Command {
  public async run() {
    const { flags: { recover } } = this.parse(EditCommand);
...
async function getContentString(content: EditorStructure, recover: boolean): Promise<string | undefined> {
...
async function onInvalidResult(err: any, originalInput: string, editorContents: string) {
