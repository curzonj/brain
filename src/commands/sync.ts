...
async function importQueues(): Promise<void> {
  ...
  const updates = await Promise.all(
    groupBy(docs, (d: models.ExistingDoc) => d.topic_id)
    .map(([id,list]: [string,models.ExistingDoc[]]) => buildQueueTopicUpdates(id, list)),
  ).then((list) => list.flat());
...
async function buildQueueTopicUpdates(topicId: string, list: models.ExistingDoc[]): Promise<models.DocUpdate[]> {
  const db = await getDB();
