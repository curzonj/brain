...
import { resetTimings, printTimings, timingAsync, timingSync } from './timing';
...
describe("editor.ts", () => {
  ...
  describe("buildEditorStructure", () => {
    it("works", async () => {
      const couchContent = await timingAsync('buildEditorStructure', () => editor.buildEditorStructure());
      const rdfContent = await timingAsync('buildEditorStructureFromRDF', () => rdfEditor.buildEditorStructureFromRDF())
        .catch(e => {
          console.log(e);
          throw e;
        });
