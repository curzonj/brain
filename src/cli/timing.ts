...
export function printTimings() {
  ...
  Object.keys(timings).forEach((name: string) => {
    ...
    process.stdout.write(util.format("timing name=%s count=%d avg=%d max=%d total=%d\n", name, a.length, avg.toFixed(2), max.toFixed(2), total.toFixed(2)));
  })
}
...
export async function timingAsync<T>(name: string, fn: () => Promise<T>): Promise<T> {
  const hrstart = process.hrtime()
