...
export async function buildEditorStructureFromRDF(): Promise<models.EditorStructure> {
  const store = await timingAsync('getRDFStore', () => getRDFStore());
  const schema = await timingAsync('getSchemaContents', () => getSchemaContents());

  const rdfTypeNode = prefix.rdf("type");
  const v1TopicNode = prefix.s("v1Topic");
  const quads = timingSync('storeGetQuads', () => store.getQuads(null, null, null, null));
  const bySubject = groupBy(quads, (q) => {
    if (isNamedNode(q.subject) && q.subject.value.startsWith(prefixes.b)) {
      return q.subject.value;
    }
  }).
  filter((pair) => pair[1].some((q) => rdfMatches(q, null, rdfTypeNode, v1TopicNode)))

  const editorDocs = timingSync('brainSubjectsMap', () => bySubject.map((pair) => {
    const list = pair[1];
    const id = termId(list[0].subject as N3.NamedNode, prefixes.b);

    const doc = timingSync('buildEditorDocForSubject', () => buildEditorDocForSubject(schema, store, list));
    return [ id, doc ];
  }));
...
function buildEditorDocForSubject(schema: DecodedSchema, store: N3.N3Store,  nodes: Quad[]): models.EditorDoc {
  const v1TopicNodes = nodes
    .filter((q) => isNamedNode(q.predicate) && q.predicate.value.startsWith(prefixes.s));

  return v1TopicNodes.reduce((acc, q) => {
    ...
    switch (fieldName) {
      ...
      case "search":
        ...
        break;
      case "links":
        const maybeTitle = store.getQuads(namedNode(value), prefix.s("title"), null, null)[0];
        if (maybeTitle) {
...
function addArrayValue(acc: models.EditorDoc, fieldName: string, value: models.Link) {
  if (!acc[fieldName]) {
    acc[fieldName] = [];
  }
