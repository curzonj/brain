...
import { buildAbstractPage, AbstractPage } from '../utils/abstract_page';
...
export function useAsync<T,V>(params: V, fn: (params: V, cb: (result: T) => void) => Promise<void | T>): T | undefined {
  const [state, setState] = useState<AsyncState<T,V>>({})
  if (state && state.params === params && state.result) return state.result;
  reportError(async () => {
    const ret: void | T = await fn(params, result => setState({ params, result }))
